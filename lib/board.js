...
function internalCompile(source){
  var TModuleRec = Board.Tokenizer.compile(source, false);
...
function listPorts(options){
  ...
  return Board.Transport.listPorts()
...
function Board(options){
  ...
  options.transport = Board.Transport;

  EventEmitter.call(this);

  this._revision = options.revision || 'bs2';
  this._protocol = new Board.Protocol(options);
  this._programmer = new Board.Programmer({
    ...
    .then(function(portList){
      return bluebird.reduce(portList, function(boardList, path){
        ...
        return bluebird.reduce(revisions, function(current, rev){
          ...
          return Board.Identify(boardOpts)
